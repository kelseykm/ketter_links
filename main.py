...
from ketter_links import scraper, drivers, search
import re
...
class scrapeSites(object):
    @staticmethod
    def o2tvseries(parsed_url, seasons_regex, episodes_regex):
    ...
    @staticmethod
    def lightdl(parsed_url, seasons_regex, episodes_regex):
    ...
    @staticmethod
    def netnaija(parsed_url, seasons_regex, episodes_regex):
...
class searchSites(object):
    @staticmethod
    def netnaija(search_string):
...
def grab_scrape_info():
    ...
    for i in args.seasons, args.episodes:
        if i is not None:
            if len(i) > 2 and not i.startswith('['):
                logger.error("Invalid input format for seasons or episodes")
                raise Exception('Invalid input format for seasons or episodes')

            for j in i:
                if not j in ''.join(['[', ']', ',', '-', ' ', string.digits]):
                    logger.error("Invalid input format for seasons or episodes")
                    raise Exception('Invalid input format for seasons or episodes')
...
def run_scrape(url, seasons_regex, episodes_regex):
    supported_sites = {
...
def parse_scrape_info(scrape_info):
    ...
    if scrape_info.strip().isdigit():
        ...
        for info in scrape_info:
            ...
            if '-' in info:
                ...
                list(separated.add(str(i)) for i in range(first, last + 1))
    ...
    raise Exception("No valid input for seasons or episodes")

def construct_scrape_regex(scrape_info):
        ...
        if info == 'seasons':
            if scrape_info[info] is not None:
                if re.search(r'/season-\d{1,6}', scrape_info['url'].geturl()):
                    ...
                    raise Exception("Season already specified in url")
        ...
        if info == 'episodes':
            if scrape_info[info] is not None:
                if re.search(r'/episode-\d{1,6}', scrape_info['url'].geturl()):
                    ...
                    raise Exception("Episode already specified in url")
...
def scrape_main():
    ...
    search_info = construct_scrape_regex(grab_scrape_info())
